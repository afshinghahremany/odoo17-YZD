...
class DynamicChequeReportWizard(models.TransientModel):
	...
	def _create_paper_format(self):
		...
		if not report_action_id:
			raise UserError(_('Someone has deleted the reference view of report, Please Update the module!'))
		config_rec = self.env['dynamic.cheque'].search([], limit=1)
		if not config_rec:
			raise UserError(_("Report format not found! Please Update Module."))
