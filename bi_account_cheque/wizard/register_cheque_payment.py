...
class RegisterChequePayment(models.TransientModel):
	...
	cheque_reference = fields.Char("Cheque Reference",required=1)
	cheque_number = fields.Char("Cheque Number",required=1)
	journal_id = fields.Many2one('account.journal',string="Payment Journal",domain=[('cheque_payment','=',True)],required=1)
	payment_date = fields.Date("Payment Date",default=fields.Date.today(),required=1)
	received_date = fields.Date("Received/Given  Date",required=1)
